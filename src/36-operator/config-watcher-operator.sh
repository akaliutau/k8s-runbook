...
get_config_watcher_for_cm() {
  ...
  local watcher_list=$(curl -s $base/apis/k8stest.xyz/v1/${ns}/configwatchers | \
                       jq -r '.items[]')
...
extract_label_selector_from_watcher() {
  ...
  local pod_selector=$(curl -s "$base/apis/k8stest.xyz/v1/${ns}/configwatchers/${watcher}" | jq .spec.podSelector)

  # Convert from JSON to 'label=value,label2=value2' as used as query parameter for the K8s API call
